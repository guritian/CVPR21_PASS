...
class protoAugSSL_for_federated:
    ...
    def pre_beforeTrain(self, classes,flag = False):
        self.model.eval()
        self.train_loader, self.test_loader = self._get_train_and_test_dataloader(classes)
        if flag:
    ...
    def pretrain(self,epochs,classes):
        ...
        for epoch in range(epochs):
            ...
            if epoch % self.args.print_freq == 0:
                accuracy = self.pretrain_test(self.test_loader,classes)
                print('epoch:%d, accuracy:%.5f' % (epoch, accuracy))
        self.protoSave(self.model, self.train_loader, 0)
    ...
    def pretrain_test(self, testloader,classes):
        self.model.eval()
        correct, total = 0.0, 0.0
        for setp, (indexs, imgs, labels) in enumerate(testloader):
            if classes[0] != 0:
    ...
    #单纯为了保存预训练的模型
    def pre_afterTrain_save(self):
        path = self.args.save_path + self.file_name + '/'
        if not os.path.isdir(path):
            os.makedirs(path)
        self.numclass = self.task_size
        filename = path + '%d_model.pkl' % (int(self.class_label[0]/10))
